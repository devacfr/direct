Ext
	.define(
		'Ext.ux.form.field.BoxSelect',
		{
			...
			alias : [
				'widget.comboboxselect', 'widget.boxselect' ],
			requires : [
				'Ext.selection.Model', 'Ext.data.Store' ],
			...
			/**
			 * @cfg {Boolean} If set to `true`, allows the combo field to hold more
			...
			/**
			 * @cfg {Boolean} `true` to activate the trigger when clicking in empty
			...
			/**
			 * @cfg {Boolean} - `true` to have each selected value fill to the width
			 ...
			 * @param {Zurich.core.form.field.BoxSelect}
			...
			initEvents : function() {
				...
				// I would prefer to use relayEvents here to forward these events on,
				// but I want
			...
			/**
			 * Overridden to search entire unfiltered store since already selected
			...
			/**
			 * Overridden to map previously selected records to the "new" versions of
			...
			/**
			 * Used to determine if a record is filtered out of the current store's
			...
			isFilteredRecord : function(record) {
				...
				filtered =
					((storeRecord === -1) && (!store.snapshot || (me.findRecord(valueField, record.get(valueField)) !== false)));

				filtered =
					filtered
						|| (!filtered && (storeRecord === -1) && (me.forceSelection !== true) && (me.valueStore.findExact(
							valueField, record.get(valueField)) >= 0));
			...
			onListSelectionChange : function(list, selectedRecords) {
				...
				if ((me.ignoreSelection <= 0) && me.isExpanded) {
					// Pull forward records that were already selected or are now filtered
					// out of the store
					valueStore.each(function(rec) {
			...
			/**
			 * Overridden to preserve scroll position of pick list when list is
			 * realigned
			 */
			...
			/**
			 * Check to see if the input field has selected text, for key-based
			 * navigation
			 * 
			 * @private
			 */
			...
			onKeyDown : function(e, t) {
				var me = this, key = e.getKey(), rawValue = me.inputEl.dom.value, valueStore = me.valueStore, selModel =
					me.selectionModel, stopEvent = false;
				...
				if (me.isExpanded && (key == e.A && e.ctrlKey)) {
					// CTRL-A when picker is expanded - add all items in current picker
					// store page to current value
					...
					var lastSelectionIndex =
						(selModel.getCount() > 0) ? valueStore.indexOf(selModel.getLastSelected() || selModel.getLastFocused())
							: -1;
					...
					if ((key == e.BACKSPACE) || (key == e.DELETE)) {
						if (lastSelectionIndex > -1) {
							...
							// select first before remove
							lastSelectionIndex = valueStore.indexOf(me.valueStore.last());
							if (lastSelectionIndex > -1) {
				...
				// Prevent key up processing for enter if it is being handled by the
				// picker
			...
			/**
			 * Handles auto-selection and creation of labelled items based on this
			...
			/**
			 * Handles auto-selection of labelled items based on this field's
			...
			onPaste : function(e, t) {
				var me = this, rawValue = me.inputEl.dom.value, clipboard =
					(e && e.browserEvent && e.browserEvent.clipboardData) ? e.browserEvent.clipboardData : false;
			...
			/**
			 * Overridden to get and set the DOM value directly for type-ahead
			 * suggestion (bypassing get/setRawValue)
			 * 
			 * @protected
			 */
			onTypeAhead : function() {
				var me = this, displayField = me.displayField, inputElDom = me.inputEl.dom, valueStore = me.valueStore, boundList =
					me.getPicker(), record, newValue, len, selStart;
			...
			/**
			 * Delegation control for selecting and removing labelled items or
			 * triggering list collapse/expansion
			 * 
			 * @protected
			 */
			onItemListClick : function(evt, el, o) {
				var me = this, itemEl = evt.getTarget('.x-boxselect-item'), closeEl =
					itemEl ? evt.getTarget('.x-boxselect-item-close') : false;
			...
			/**
			 * Build the markup for the labelled items. Template must be built on
			...
			/**
			 * @inheritdoc Intercept calls to getRawValue to pretend there is no
			...
			/**
			 * @inheritdoc Intercept calls to setRawValue to pretend there is no
			 *             inputEl for rawValue handling, so that we can use inputEl
			 *             for user input of just the current value.
			 */
			...
			/**
			 * Sets the specified value(s) into the field. The following value formats
			...
			setValue : function(value, doSelect, skipLoad) {
				...
				if (value && value.isStore && me.multiSelect) {
					me.valueStore.loadRecords(value.data.items);
			...
			/**
			 * @inheritdoc Overridden to optionally allow for submitting the field as
			...
			/**
			 * Overridden to clear the input field if we are auto-setting a value as
			 * we blur.
			 * 
			 * @protected
			 */
			...
			assertValue : function() {
				var me = this, rawValue = me.inputEl.dom.value, rec =
					!Ext.isEmpty(rawValue) ? me.findRecordByDisplay(rawValue) : false, value = false;
			...
			/**
			 * Expand record values for evaluating change and fire change events for
			 * UI to respond to
			 */
			checkChange : function() {
				if (!this.suspendCheckChange && !this.isDestroyed) {
					var me = this, valueStore = me.valueStore, lastValue = me.lastValue, valueField = me.valueField, newValue =
						Ext.Array.map(Ext.Array.from(me.value), function(val) {
							if (val.isModel) {
			...
			/**
			 * Intercept calls to renderActiveError to add invalidCls, because the
			...
			/**
			 * Track height change to fire {@link #event-autosize} event, when
			 * applicable.
			 */
 ...
 * {@link Zurich.core.form.field.BoxSelect#multiSelect} or
 * {@link Zurich.core.form.field.BoxSelect#grow} is false.
 */
Ext.define('Ext.ux.layout.component.field.BoxSelectField', {
	...
	publishInnerWidth : function(ownerContext) {
		var me = this, owner = me.owner, width = owner.itemList.getWidth(true) - 10, lastEntry =
			owner.inputElCt.prev(null, true);
