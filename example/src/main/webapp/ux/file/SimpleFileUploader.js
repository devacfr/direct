/**
 * A simple UI for file uploading that shows a "Browse" button. Will also allow
 * file(s) to be dropped on the container.
 */
Ext.define('Ext.ux.file.SimpleFileUploader', {
	extend : 'Ext.container.Container',
	requires : [ 'Ext.container.Container', 'Ext.layout.container.VBox', 'Ext.ux.file.FileUploader',
			'Ext.ux.file.FileManager', 'Ext.ux.file.FileItem' ],
	config : {
		/**
		 * @cfg {String} url
		 * 
		 ...
		 * and must have the same signature as specified for that object's
		 * {@link Cs.file.data.FileUploader#uploadWith} method.
		 * 
		 ...
		 * 
		 * A config to apply to each {@link Cs.file.ui.FileItem} component
	 ...
	 * 
	 * @param {Object}
	 *            config The config to apply to this component. A `vbox` layout
	...
	initComponent : function() {
		var me = this, fileMap = {}, uploadCmp, uploadCmpDef = {
			xtype : 'container',
			height : 28,
			anchor : "100%",
			padding : "3 0 0 0",
			...
			hidden : true
		}, listCmp, fileMgr = Ext.create('Ext.ux.file.FileManager'), uploader, uploaderDef = {
			url : me.getUrl()
		};
		...
		fileMgr.on('fileadded', function(record) {
			var item = Ext.create('Ext.ux.file.FileItem', record.get('name'), record.get('size'), me.getItemConfig()
				|| {
					height : 25,
					style : {
						'padding-top' : 1,
		...
		uploader = Ext.create('Ext.ux.file.FileUploader', fileMgr, Ext.apply(uploaderDef, {
		...
		this.on('afterrender', function(c, opt) {
			var containerEl = me.container, msgCmp, fileCmp, fileCmpDef = {
				xtype : 'filefield',
				anchor : "100%",
				height : 30,
