...
Ext.define('Ext.ux.toggleslide.ToggleSlide', {
	...
	requires : [
		'Ext.ux.toggleslide.Thumb', 'Ext.fx.Anim' ],
	...
	/**
	 * @cfg {Boolean} resizeContainer Specifies whether the contain element should
	 *      be resized (defaults to true)
	 */
	...
	initComponent : function() {
		 ...
		 * @param {Ext.form.Checkbox}
		 *          this This toggle
		 * @param {Boolean|String}
		 *          state The next toggle state value if boolean mode else the label
		 *          for the next state
		 */
		'beforechange',
		 ...
		 *          this This toggle
		 * @param {Boolean|String}
		 *          state the new toggle state value, boolean if in boolean mode
		 *          else the label
		 */
		'change');
	},
	...
	onRender : function() {
		...
		me.thumb = new Zurich.core.toggleslide.Thumb({
			ownerCt : me,
			slider : me,
		...
		if (!me.disabled) {
			...
			Zurich.core.toggleslide.ToggleSlide.superclass.disable.call(me);
		}
	},
	...
	disableTextSelection : function() {
		var els = [
			this.el, this.onLabel, this.offLabel, this.handle ];
	 ...
	 * position. Toggle if so or move the handle back to the original position if
	 * not.
	 * 
	 * @private
	 */
	onDragEnd : function(e) {
		var me = this, hc = (me.handle.getLeft(true) + me.handle.getRight(true)) / 2, cc =
			(me.el.getLeft(true) + me.el.getRight(true)) / 2, next = hc > cc;
	...
	enable : function() {
		if (this.disabled) {
			Zurich.core.toggleslide.ToggleSlide.superclass.enable.call(this);
	...
	disable : function() {
		if (!this.disabled) {
			Zurich.core.toggleslide.ToggleSlide.superclass.disable.call(this);
	 ...
	 * @param value
	 */
	// TODO [devacfr] add setValue function
	setValue : function(value) {
		...
		if (me.fireEvent('beforechange', me, v) !== false) {
			me.state = v;
			if (me.rendered) {
				me.moveHandle(me.state, Ext.bind(me.fireEvent, me, [
					'change', me, me.getValue() ]));
