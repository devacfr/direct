Ext.define('Ext.ux.file.UglyFileUploader', {
	extend : 'Ext.container.Container',
	requires : [ 'Ext.container.Container', 'Ext.ux.file.FileUploader', 'Ext.ux.file.FileItem' ],
	alias : 'widget.uglyfileuploader',
	config : {
		mgr : null
	},
	constructor : function(config) {
		var listContainer, me = this, fileItems = {}, form, fileMgr = config["mgr"] || null, uploader = Ext
			.create('Ext.ux.file.FileUploader', fileMgr,
				{
					url : "upload.php",
					progress : function(file, total, amt, evt) {
						if (window['console']) {
							console.log("Uploaded " + Ext.Number.toFixed(100 * amt / total, 2) + " of "
								+ file.get('name'));
						}
					},
		...
		fileMgr.on('fileadded', function(record) {
			var item = Ext.create('Ext.ux.file.FileItem', record.get('name'), record.get('size'));
				...
				items : [ {
					...
					listeners : {
						change : function(field, value, opt) {
							var inputCnt = field.ownerCt, inputEl = field.fileInputEl;
							...
							if (inputEl.dom.files) {
								...
								inputCnt.add(field.cloneConfig({
									buttonText : "Add a File ... (" + count + ")"
								}));
